...
// Dados simulados de formulários PDF
const mockPdfs = [
  {
    id: "1",
...
export function PdfList() {
  ...
  // Filtrar PDFs com base no termo de pesquisa
  ...
  // Simular download do PDF com animação
  const handleDownload = (id: string, patientName: string, formName: string) => {
    setDownloadingId(id)

    // Simular tempo de download
    setTimeout(() => {
      toast.success("Download concluído", `${formName} - ${patientName}`)
          ...
          <Input
            ...
            onChange={(e) => {
              ...
              if (e.target.value && e.target.value.length > 2) {
                toast.info("Pesquisando", `Buscando por "${e.target.value}"`, 2000)
              }
            }}
        ...
        <div className="rounded-md border border-gray-200 dark:border-gray-700">
          <Table>
            <TableHeader className="bg-gray-100 dark:bg-gray-800">
              <TableRow>
                <TableHead className="text-gray-700 dark:text-gray-300">Nome do Paciente</TableHead>
                <TableHead className="text-gray-700 dark:text-gray-300">Formulário</TableHead>
            ...
            <TableBody>
              {filteredPdfs.length > 0 ? (
                filteredPdfs.map((pdf, index) => (
                  ...
                  >
                    <TableCell className="font-medium text-gray-800 dark:text-gray-300">{pdf.patientName}</TableCell>
                    <TableCell className="text-gray-700 dark:text-gray-400">{pdf.formName}</TableCell>
                    <TableCell className="text-gray-700 dark:text-gray-400">{formatDate(pdf.date)}</TableCell>
                    <TableCell className="text-right">
                      <Button
                        variant="ghost"
                        size="sm"
                        onClick={() => handleDownload(pdf.id, pdf.patientName, pdf.formName)}
                        disabled={downloadingId === pdf.id}
                        className={`text-gray-700 hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-gray-700 hover-lift ${downloadingId === pdf.id ? "animate-pulse-purple" : ""}`}
                      >
                        <FaDownload
                          className={`mr-2 ${downloadingId === pdf.id ? "text-purple-500 dark:text-purple-400" : "text-purple-600 dark:text-purple-400"}`}
                        />

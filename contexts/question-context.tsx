...
type QuestionContextType = {
  ...
  updateQuestion: (id: string, data: UpdateQuestionDTO) => Promise<Question | null>
...
export function QuestionProvider({ children }: { children: React.ReactNode }) {
  ...
  const [hierarchicalQuestions, setHierarchicalQuestions] = useState<Question[]>([])
  ...
  const loadQuestionsByQuestionnaire = useCallback(async (questionnaireId: string) => {
    setIsLoading(true)
    setError(null)

    try {
      const data = await QuestionService.getQuestionsByQuestionnaire({ questionnaireId })
      setQuestions(data)

      // Organizar questões em hierarquia
      const organized = QuestionService.organizeQuestionsHierarchy(data)
      setHierarchicalQuestions(organized)

      return data
    } catch (err: any) {
      const errorMessage = err.response?.data?.message || "Erro ao carregar questões"
  ...
  const getQuestion = useCallback(
    async (id: string) => {
      ...
      try {
        ...
        const errorMessage = err.response?.data?.message || "Erro ao buscar questão"
        setError(errorMessage)
  ...
  const createQuestion = useCallback(
    async (data: CreateQuestionDTO) => {
      ...
      try {
        ...
        // Atualizar o estado com a nova questão
        setQuestions((prev) => [...prev, newQuestion])

        // Reorganizar a hierarquia
        const updatedQuestions = [...questions, newQuestion]
        const organized = QuestionService.organizeQuestionsHierarchy(updatedQuestions)
        setHierarchicalQuestions(organized)

        toast.success("Sucesso", "Questão criada com sucesso")
        return newQuestion
      } catch (err: any) {
        const errorMessage = err.response?.data?.message || "Erro ao criar questão"
        setError(errorMessage)
        toast.error("Erro", errorMessage)
        return null
      } finally {
  ...
  const updateQuestion = useCallback(
    async (id: string, data: UpdateQuestionDTO) => {
      ...
      try {
        ...
        // Atualizar o estado com a questão atualizada
        ...
        const errorMessage = err.response?.data?.message || "Erro ao atualizar questão"
        setError(errorMessage)
        toast.error("Erro", errorMessage)
        return null
      } finally {
  ...
  const deleteQuestion = useCallback(
    async (id: string) => {
      ...
      try {
        ...
        // Atualizar o estado removendo a questão excluída
        ...
        const errorMessage = err.response?.data?.message || "Erro ao excluir questão"
        setError(errorMessage)
        toast.error("Erro", errorMessage)
        return false
      } finally {

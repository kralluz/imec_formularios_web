...
export default function SectorsPage() {
  const { sectors, loadSectors, createSector, updateSector, deleteSector, isLoading, hasLoaded } = useSector()
  const toast = useCustomToast()

  const [isDialogOpen, setIsDialogOpen] = useState(false)
  const [isEditMode, setIsEditMode] = useState(false)
  const [currentSector, setCurrentSector] = useState<{ id?: string; name: string }>({ name: "" })
  const [isSubmitting, setIsSubmitting] = useState(false)

  // Carregar setores apenas uma vez quando a página é montada
  useEffect(() => {
    if (!hasLoaded) {
      loadSectors()
    }
  ...
  const handleOpenEditDialog = (sector: { id?: string; name: string }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (!currentSector.name.trim()) {
        toast.error("Erro", "O nome do setor é obrigatório")
  ...
  const handleDeleteSector = async (id?: string, name?: string) => {
    if (!id) {
      toast.error("Erro", "ID do setor não encontrado")
      return
    }
    ...
    <div className="animate-slide-up opacity-0">
      <div className="mb-6 flex items-center justify-between">
        <h1 className="text-3xl font-bold text-gray-800 dark:text-white">
          <span className="text-purple-600 dark:text-purple-400">Gerenciar Setores</span>
        </h1>
        <Button
                ...
                <TableBody>
                  {sectors.length > 0 ? (
                    sectors.map((sector) => (
                      ...
                      >
                        <TableCell className="font-medium text-gray-800 dark:text-gray-300">{sector.name}</TableCell>
                        ...
                        <TableCell className="text-right">
                          <div className="flex justify-end space-x-2">
                            <Button
                              variant="ghost"
                              size="sm"
                              onClick={() => handleOpenEditDialog({ id: sector.id, name: sector.name })}
                    ...
                    <TableRow>
                      <TableCell colSpan={3} className="h-24 text-center text-gray-700 dark:text-gray-400">
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              {isEditMode ? "Atualize as informações do setor" : "Preencha os dados para criar um novo setor"}
            </DialogDescription>
          </DialogHeader>
